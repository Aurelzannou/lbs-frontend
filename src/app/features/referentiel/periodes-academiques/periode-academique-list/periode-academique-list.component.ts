...
@Component({
  selector: 'app-periode-academique-list',
  standalone: true,
  imports: [
    CommonModule, 
    MatTableModule, 
  ...
  templateUrl: './periode-academique-list.component.html',
  styleUrl: './periode-academique-list.component.scss'
})
export class PeriodeAcademiqueListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  async deletePeriode(periode: PeriodeAcademique): Promise<void> {
    const confirmed = await this.notification.confirm(`Souhaitez-vous vraiment supprimer la période ${periode.libelle} ?`);
    if (confirmed) {
      this.loading = true;
      this.periodeService.delete(periode.uuid!).subscribe({
        next: () => {
          this.notification.success('Période supprimée avec succès');
          this.refresh();
        },
        error: () => {
