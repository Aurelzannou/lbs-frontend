...
import { TypeActeFormDialogComponent } from '../type-acte-form-dialog/type-acte-form-dialog.component';
import { Subject, Subscription } from 'rxjs';
import { debounceTime, distinctUntilChanged } from 'rxjs/operators';
import { animate, style, transition, trigger } from '@angular/animations';

@Component({
  selector: 'app-type-acte-list',
  standalone: true,
  imports: [
    CommonModule, 
    MatTableModule, 
  ...
  templateUrl: './type-acte-list.component.html',
  styleUrl: './type-acte-list.component.scss'
})
export class TypeActeListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  refresh(): void {
    this.loading = true;
    this.typeActeService.getAll(this.pageIndex + 1, this.pageSize, this.searchTerm).subscribe({
      next: (response: any) => {
      ...
      error: (err) => {
        console.error('Erreur chargement types d\'actes:', err);
        this.notification.error('Impossible de charger les types d\'actes');
        this.loading = false;
      }
    });
  }
  ...
  async deleteTypeActe(typeActe: TypeActe): Promise<void> {
    const confirmed = await this.notification.confirm(`Souhaitez-vous vraiment supprimer le type d'acte ${typeActe.libelle} ?`);
    if (confirmed) {
      this.loading = true;
      this.typeActeService.delete(typeActe.uuid!).subscribe({
        next: () => {
          this.notification.success('Type d\'acte supprimé avec succès');
          this.refresh();
        },
        error: () => {
