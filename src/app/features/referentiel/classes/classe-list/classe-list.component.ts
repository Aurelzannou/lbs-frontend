...
import { ClasseFormDialogComponent } from '../classe-form-dialog/classe-form-dialog.component';
...
export class ClasseListComponent implements OnInit, OnDestroy, AfterViewInit {
  private classeService = inject(ClasseService);
  private notification = inject(NotificationService);
  private dialog = inject(MatDialog);
  private cdr = inject(ChangeDetectorRef);

  displayedColumns: string[] = ['code', 'libelle', 'niveau', 'actions'];
  ...
  refresh(): void {
    this.loading = true;
    this.classeService.getAll(this.pageIndex + 1, this.pageSize, this.searchTerm).subscribe({
      next: (response: any) => {
      ...
      error: (err) => {
        ...
        this.notification.error('Impossible de charger les classes');
        this.loading = false;
      }
    });
  }
  ...
  async deleteClasse(classe: Classe): Promise<void> {
    const confirmed = await this.notification.confirm(`Souhaitez-vous vraiment supprimer la classe ${classe.libelle} ?`);
    if (confirmed) {
      this.loading = true;
      this.classeService.delete(classe.uuid!).subscribe({
        next: () => {
