...
import { ModePaiementFormDialogComponent } from '../mode-paiement-form-dialog/mode-paiement-form-dialog.component';
...
@Component({
  ...
  styleUrl: './mode-paiement-list.component.scss'
})
export class ModePaiementListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  refresh(): void {
    this.loading = true;
    this.modePaiementService.getAll(this.pageIndex + 1, this.pageSize, this.searchTerm).subscribe({
      next: (response: any) => {
      ...
      error: (err) => {
        console.error('Erreur chargement modes paiements:', err);
        this.notification.error('Impossible de charger les modes de paiement');
        this.loading = false;
      }
    });
  }
  ...
  async deleteMode(mode: ModePaiement): Promise<void> {
    const confirmed = await this.notification.confirm(`Souhaitez-vous vraiment supprimer le mode de paiement ${mode.libelle} ?`);
    if (confirmed) {
      this.loading = true;
      this.modePaiementService.delete(mode.uuid!).subscribe({
        next: () => {
          this.notification.success('Mode de paiement supprimé avec succès');
          this.refresh();
        },
        error: () => {
