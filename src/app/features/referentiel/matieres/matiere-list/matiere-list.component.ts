...
import { MatiereFormDialogComponent } from '../matiere-form-dialog/matiere-form-dialog.component';
...
export class MatiereListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  refresh(): void {
    this.loading = true;
    this.matiereService.getAll(this.pageIndex + 1, this.pageSize, this.searchTerm).subscribe({
      next: (response: any) => {
      ...
      error: (err) => {
        ...
        this.notification.error('Impossible de charger les matières');
        this.loading = false;
      }
    });
  }
  ...
  async deleteMatiere(matiere: Matiere): Promise<void> {
    const confirmed = await this.notification.confirm(`Souhaitez-vous vraiment supprimer la matière ${matiere.libelle} ?`);
    if (confirmed) {
      this.loading = true;
      this.matiereService.delete(matiere.uuid!).subscribe({
        next: () => {
          this.notification.success('Matière supprimée avec succès');
          this.refresh();
        },
        error: () => {
